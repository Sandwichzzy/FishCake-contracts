...
interface IStakingManager {
    event StakeHolderDepositStaking(address indexed staker, uint256 stakingAmount, uint256 messageNonce);

    event StakeHolderWithdrawStaking(
        address indexed recipant, uint256 withdrawAmount, uint256 messageNonce, bytes32 messageHash
    );
